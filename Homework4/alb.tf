resource "aws_alb" "web-alb" {
  name = "app-alb-${data.tfe_outputs.vpc.values.vpc_id}"
  load_balancer_type = "application"
  dynamic "subnet_mapping" {
    for_each = data.tfe_outputs.vpc.values.public_subnets_id
    content {
  ...
  tags = {
    Name = "app_alb ${data.tfe_outputs.vpc.values.vpc_id}"
  }
}
...
resource "aws_alb_target_group" "web-group" {
  ...
  health_check {
   enabled = true
    ...
    tags = {
    "Name" = "web-target-group-${data.tfe_outputs.vpc.values.vpc_id}"
  }
  stickiness {
    type = "lb_cookie"
    cookie_duration = 60
